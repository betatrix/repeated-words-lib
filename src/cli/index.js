...
program
  ...
  .option(
    ...
    "Destination path for results file",
    ".",
  )
  .option(
    "-n, --name-text <string>",
  ...
  .action((options) => {
    ...
    if (!text) {
      console.error(
        chalk.red(
          "Error: flag -t is missing. Type --help for additional information",
        ),
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        chalk.red("Error: An error occurred while processing the file", error),
      );
    }
  });
